...
contract EXPToken is ERC20, Ownable, ISoulbound {
    /**
     * ==================================================================
     *                          STATE VARIABLES
     * ==================================================================
    ...
    constructor(
        ...
        string memory sTokenSymbol //, address aRrpAirnode)
    )
        ERC20(sTokenName, sTokenSymbol) //QRNGRequester(aRrpAirnode)
    {
        ...
        onlyOwner
    {
        ...
        if (aWhichAddress == address(0)) {
            revert InvalidAddress();
        }
        // Check if operation actually has any effects or should
        // be disregarded
        if (mTokenAdmins[aWhichAddress] == bIsAdmin) {
            revert ActionHasNoEffects();
        }
        // Set token admin
    ...
    function gainExperience(address aWhichPlayer, uint256 iHowMuchGained)
        public
    {
        // Make sure only token admins can call this function
        if (!mTokenAdmins[_msgSender()]) {
            revert OnlyTokenAdminsAllowed();
        }
        ...
        require(
            _bal + iHowMuchGained <= MAXEXP,
            "EXPToken (Balance): Will go above Max(100)."
        );
